...
<template>
    ...
    <div class="container mx-auto px-6 pb-12 sm:px-0">
        ...
        <div class="space-x-4 text-center">
            <button v-if="!($route.query.setup == 'type' && $route.query.question == 1)" class="btn w-40 mb-3" @click="goType()">上一步</button>
            <button v-if="!($route.query.setup == 'range' && $route.query.question == 3)" class="btn w-40" @click="goRange()">下一步</button>
    ...
    setup() {
        ...
        const goRange = () => {
            ...
            if (isAllWrite === 1) {
                if (route.query.question != 3) {
                    router.push({ path: "/dataType/v2", query: { setup: route.query.setup , question: parseInt(route.query.question)+1 } });
                }else {
                    router.push({ path: "/dataType/v2", query: { setup: "range" , question: 1} });
                    showContSub.value = dataContSub.value.filter(item => item.DC_MAIN == "DM02");
                }
            }
            else
        ...
        const goType = () => {
            if (route.query.setup == "type") {
                router.push({ path: "/dataType/v2", query: { setup: "type", question: parseInt(route.query.question)-1 } });
            }else if (route.query.setup == "range") {
                if (route.query.question == 1) {
                    router.push({ path: "/dataType/v2", query: { setup: "type", question: 3 } });
                    showContSub.value = dataContSub.value.filter(item => item.DC_MAIN == "DM01");
                }else {
                    router.push({ path: "/dataType/v2", query: { setup: "range", question: parseInt(route.query.question)-1 } });
                }
            }
        };
        // ---------- api
