...
namespace WebApplication.Models.General
{
    ...
    public class GetImageCode
    {
        ...
        public ImageCode GetImage()
        {
            ...
            imageCode.Code = code;
            image.Dispose();
            HttpContext.Current.Session["ImageCode"] = JsonConvert.SerializeObject(imageCode);
            imageCode.Code = "";
            return imageCode;
        }
        ...
        /// <returns></returns>
        public bool IsAuthorizedCode(string code)
        {
            bool isAuthorized = false;

            if (Current.Session != null && HttpContext.Current.Session["ImageCode"] != null)
            {
                try
                {
                    ImageCode imageCode = JsonConvert.DeserializeObject<ImageCode>(FixFunc.FixEncode(FixFunc.FixCrossSiteScripting(HttpContext.Current.Session["ImageCode"])));

                    if (imageCode != null && imageCode.Code == code)
                        isAuthorized = true;
                }
                catch (Exception ex)
                {
                    new NLog().LogDetail("Mortuary", "ImageCode", "IsAuthorizedCode", ex.Message);
                }
            }
